...
def neighbor_index_generator(im_h, im_w, beta, batch_size):
    ...
    return tf.tile(uv_index_bucket, (batch_size, 1, 1, 1, 1))
...
def depth_to_pointcloud(depth_map, im_h, im_w, batch_size):
    ...
    xv = tf.tile(xv,(batch_size, 1, 1, 1))
    yv = tf.tile(yv,(batch_size, 1, 1, 1))
    depth_map = tf.expand_dims(depth_map, axis=-1)
    pointcloud = tf.concat([xv, yv, depth_map], axis = 3)
    return pointcloud


def kernel_regression(depth_map, normal_map, beta, delta):
    batch_size, im_h, im_w = depth_map.get_shape().as_list()
    window_size = beta*2 + 1
    ...
    zero_tensor = tf.constant(0, dtype=tf.float32, shape=normal_similarity.get_shape().as_list())
