...
class DataGenerator():
    ...
    def _aux_generator(self, dummy_input=0):
        """ Auxiliary Generator
        Args:
        ...
        generated_batch['gtnormal'] = np.zeros([batch_size, self.normalres[0], self.normalres[1], 3],
                                               dtype=np.float32)
        i = 0
        if datatype == 'normal_dataset':
            ...
            generated_batch['normal_train_gtdepthre'] = np.zeros((batch_size, self.normalres[0], self.normalres[1]),
                                                                 dtype=np.float32)
            generated_batch['train_mask'] = np.zeros([batch_size, self.normalres[0], self.normalres[1]],
                                                     dtype=np.bool)
            ...
            while i < batch_size:
                ...
                generated_batch['train_mask'][i] = generated_mask
        ...
        while i < batch_size:
            ...
            if datatype != 'detail_data':
                ...
                # depth_full = np.load(name[0:-8] + '_depth.npy') * 1000
            ...
            if datatype != 'detail_data':
                ...
                #     self.rot = 0
                #     self.scale = util.getScale_detail(depth_full)
                #     self.center = util.getCenter_detail(depth_full)
                # center = self.center
                # rot = self.rot
                # scale = self.scale
                rot = 0
            ...
            if self.show:
                imgnew = Image.fromarray(img.astype(np.uint8), 'RGB')
                imgnew.show()
            ...
            if datatype == 'detail_data':
                ...
                ori_mask = depm_continue > 100
                ori_mask = ndimage.binary_erosion(ori_mask).astype(ori_mask.dtype)
                ori_mask = ndimage.binary_erosion(ori_mask).astype(ori_mask.dtype)
                ori_mask = ndimage.binary_dilation(ori_mask).astype(ori_mask.dtype)
                ori_mask = ndimage.binary_dilation(ori_mask).astype(ori_mask.dtype)
                depm_continue[ori_mask==False] = 0

                depm_continue = depm_continue - np.median(depm_continue[depm_continue > 1]) + 2000
                depm_continue[depm_continue<1000] = 0
                ...
                # calculate ground true normal from depth

                # ori_mask = depth_full > 100

                # start = datetime.now()
                # pcd = PointCloud(np.expand_dims(depth_full, 0), np.expand_dims(ori_mask, 0))
                # # normal_util.dump_pointcloud(pcd.data, 'ori_pointcloud')
                # ori_normal = pcd.get_normal().squeeze(0)
                # gt_normal = util_detail.cropfor3d(ori_normal,center,scale,rot,self.normalres[0],'nearest')
                # gt_normal = normal_util.normalize(gt_normal)
                # generated_batch['gtnormal'][i, :, :] = gt_normal
                # try:
                #     generated_batch['gtnormal'][i] = np.load(name[0:-8] + '_normal.npy')
                # except:
                #     self.currentindex += 1
                #     continue
                # end = datetime.now()
...
if __name__ == '__main__':
    ...
    seq_dir = '/media/vision-gpu/OLD/humen_depth_data/detail_data/train_data/all'
    meanRgb_dir = '/media/vision-gpu/OLD/humen_depth_data/meanRgb.t7'
    result_dir = '/media/vision-gpu/OLD/humen_depth_data/detail_data/tensorlayer'
    ...
    dataset_type = 'detail_data'
    data_split_type = 'test_seq'
    test_seqs = sorted(os.listdir(seq_dir))
    total_counting = 0
    for test_seq in test_seqs:
        test_dir = os.path.join(seq_dir, test_seq)
        num_data = len(glob2.glob(test_dir + '/*_rgb.png'))
        print('number of data:', num_data)

        random.seed(1)
        generator = DataGenerator(train_dir, valid_dir, test_dir, bg_dir, meanRgb_dir, False, True, 1, data_split_type
                                                , dataset_type, 256, 64)
        generator._reset_filelist()
        if not os.path.exists(result_dir):
            os.mkdir(result_dir)
        save_rgb_results_dir = os.path.join(result_dir, 'rgb_train')
        save_depth_results_dir = os.path.join(result_dir, 'depth_train')


        for bi in range(num_data):
            if (bi + 1) % 10 == 0:
                print('processing batch no. {}'.format(bi + 1))
            batch = generator._aux_generator(1)
            img_batch = batch['train_img']
            depth_batch = batch['train_gtdepthre']

            for i in range(1):
                cv2.imwrite(os.path.join(save_rgb_results_dir, '{:05}_rgb.png'.format(total_counting)), img_batch[i][:,:,::-1].astype(np.uint8))
                cv2.imwrite(os.path.join(save_depth_results_dir, '{:05}_rgb.png'.format(total_counting)), depth_batch[i].astype(np.uint16))
                a = cv2.imread(os.path.join(save_depth_results_dir, '{:05}_rgb.png'.format(total_counting)))
                total_counting +=1
